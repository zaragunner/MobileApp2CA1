...
fun uploadImageView(app: BookingApp, imageView: ImageView) {
...
fun serviceAvailableMessage(activity: FragmentActivity) {
    Toast.makeText(activity,
        "Booking Contacted Successfully",
        Toast.LENGTH_LONG
    ).show()
}
...
fun writeImageRef(app: BookingApp, imageRef: String) {
...
fun updateAllBookings(app: BookingApp) {
    val userId = app.auth.currentUser!!.uid
...
fun validatePhoto(app: BookingApp, activity: Activity) {
...
fun checkExistingPhoto(app: BookingApp,activity: Activity) {
